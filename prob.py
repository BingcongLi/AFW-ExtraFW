...
class l1_constraint():
    ...
    def projection(self, point):
        """
        projection on to l1 norm ball
        using code from https://gist.github.com/daien/1272551
        """
        return euclidean_proj_l1ball(point, self.R)
...
class l2_constraint():
    ...
    def projection(self, point):
        ...
        projection on to l1 norm ball
        Args:
            point: a point that may live outside the l2 norm ball constraint
        """
        point_norm = np.linalg.norm(point)
        if point_norm <= self.R:
            # x lives inside the norm ball
            scale_factor = 1.0
        else:
            # x is outside the norm ball
            scale_factor = self.R / point_norm
...
class n_supp_constraint():
    ...
    def projection(self, point):
        raise ValueError('Please let the authors know if there is an efficient implementation for projecting onto n-supp norm balls')
